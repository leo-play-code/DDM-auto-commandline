...
multiple_task = 83
if version == 'macos':
    cmd_cd = 'cd /Applications/DDM'
    cmd_mkdir ='mkdir temp'
    cmd_logcreate ='./DDM /Log /Applications/DDM/temp/DDM.txt'
    cmd_logfile = '/Applications/DDM/temp/DDM.txt'
    cmd_tempfile = '/Applications/DDM/temp.txt'
    cmd_dialogfile = '/Applications/DDM/temp/ddmdiag.log'
    cmd_getAttritube ='./DDM /ReadAssetAttributes'
    cmd_create_dialog ='./DDM /SaveDiagnosticReport /Applications/DDM/temp/ddmdiag.log'
    terminal_cmd = 'open -a Terminal'
else:
    cmd_cd = 'cd C:\Program Files\Dell\Dell Display Manager 2.0'
    cmd_mkdir ='mkdir C:\\temp'
    cmd_logcreate ='ddm /Log C:\\temp\ddm.txt'
    cmd_logfile = 'C:\\temp\ddm.txt'
    cmd_tempfile = 'C:\\temp.txt'
    cmd_dialogfile = 'c:\\temp\ddmdiag.log'
    cmd_getAttritube ='ddm /ReadAssetAttributes'
    cmd_create_dialog ='ddm /SaveDiagnosticReport c:\\temp\ddmdiag.log'
    terminal_cmd = 'start cmd'
...
class main(QMainWindow):
    def __init__(self):
        ...
        self.startbutton.setGeometry(self.x*0.13,self.y*0.85, self.x*0.1, 80)
        self.startbutton.clicked.connect(self.start)  
        # 暫停按鈕
        self.pause_button=QPushButton('暫停',self)
        self.pause_button.setIcon(self.style().standardIcon(QStyle.SP_MediaPause))
        self.pause_button.setGeometry(self.x*0.13,self.y*0.85, self.x*0.1, 80)
        self.pause_button.clicked.connect(self.pause)
        self.pause_button.hide()
        # 結束
        self.end_button=QPushButton('結束',self)
        self.end_button.setIcon(self.style().standardIcon(QStyle.SP_MediaStop))
        self.end_button.setGeometry(self.x*0.23,self.y*0.85, self.x*0.1, 80)
        ...
        self.keep_going_button.setGeometry(self.x*0.13,self.y*0.85, self.x*0.1, 80)
        ...
        self.detect_button.setGeometry(self.x*0.03,self.y*0.85, self.x*0.1, 80)
        self.detect_button.clicked.connect(self.ddmbutton)
        # googlesheet 複製
        self.googlesheet=QPushButton('googlesheet',self)
        self.googlesheet.setGeometry(self.x*0.85,self.y*0.85, self.x*0.1, 80)
    ...
    def create_new_monitor_to_qlineedit(self):
        ...
        self.label_new_monitor.setGeometry(self.x*0.03, self.y*(self.top_set+0.05*9), self.x*0.15, 30)
        self.set_new_monitor = QLineEdit('',self)
        self.set_new_monitor.setPlaceholderText("輸入新monitor：")
        self.set_new_monitor.setGeometry(self.x*0.05, self.y*(self.top_set+0.05*10), self.x*0.15, 30)
        self.set_new_monitor.textChanged.connect(lambda: self.new_monitor_name())
        self.create_monitor=QPushButton('建立', self)
        self.create_monitor.setGeometry(self.x*0.05,self.y*(self.top_set+0.05*11),self.x*0.15, 30)
    ...
    def create_new_monitor(self):
        ...
        print(new_list)
        self.label_monitor = QLabel("選擇monitor:",self)
        self.label_monitor.setGeometry(self.x*0.03, self.y*(self.top_set+0.05*9), self.x*0.2, 30)
        self.choose_monitor_label = QLabel('main：',self)
        self.choose_monitor_label.setGeometry(self.x*0.04, self.y*(self.top_set+0.05*9.9), self.x*0.1, 30)
        self.choose_monitor.addItems(new_list)
        self.choose_monitor.currentIndexChanged.connect(self.save_monitor)
        self.choose_monitor.setEditable(True)
        self.choose_monitor.setGeometry(self.x*0.1, self.y*(self.top_set+0.05*10), self.x*0.15, 30)
        self.create_new_monitor_button=QPushButton('建立新monitor', self)
        self.create_new_monitor_button.setGeometry(self.x*0.1,self.y*(self.top_set+0.05*12),self.x*0.15, 30)
        self.create_new_monitor_button.clicked.connect(self.create_new_monitor_to_qlineedit)
        # 附螢幕
        self.choose_second_monitor_label = QLabel('second：',self)
        self.choose_second_monitor_label.setGeometry(self.x*0.04, self.y*(self.top_set+0.05*11), self.x*0.2, 30)
        self.choose_second_monitor = QComboBox(self)
        self.choose_second_monitor.addItems(new_list)
        self.choose_second_monitor.currentIndexChanged.connect(self.save_second_monitor)
        self.choose_second_monitor.setEditable(True)
        self.choose_second_monitor.setGeometry(self.x*0.1, self.y*(self.top_set+0.05*11), self.x*0.15, 30)
        #
                ...
                if len(dictlist)>0:
                    if i not in skip:
                        ...
                        while j<len(dictlist):
                            ...
                            if 'WriteActiveInput' in cmd and i<multiple_task:
                                cmdlist=cmdlist+cmd+'\n'
                                cmdans=self.runcmd(cmdans,cmd)
                                if cmdans == False:
                                    return 
                                if version == 'macos':
                                    getcmd = './DDM /ReadActiveInput'
                                else:
                                    getcmd = 'DDM /ReadActiveInput'
                                try:
                                    sheet.update_cell(len(sheet.get_all_values()), 4, cmdlist)
                                ...
                                try:
                                    ...
                                    print('error=2343',e)
                                    internet_bool = False
                            elif 'WriteActiveInput' in cmd and i>multiple_task:
                                cmdlist=cmdlist+cmd+'\n'
                                cmdans=self.runcmd(cmdans,cmd)
                                if cmdans == False:
                                    return 
                                if version == 'macos':
                                    getcmd = './DDM /1:ReadActiveInput /2:ReadActiveInput'
                                else:
                                    getcmd = 'DDM /1:ReadActiveInput /2:ReadActiveInput'
                                try:
                                    sheet.update_cell(len(sheet.get_all_values()), 4, cmdlist)
                                ...
                                try:
                                    ...
                                    print('error=2343',e)
                                    internet_bool = False
                            elif 'WriteColorPreset' in cmd and i<multiple_task:
                                cmdlist=cmdlist+cmd+'\n'
                                cmdans=self.runcmd(cmdans,cmd)
                                if cmdans == False:
                                    return 
                                if version == 'macos':
                                    getcmd = './DDM /ReadColorPreset'
                                else:
                                    getcmd = 'DDM /ReadColorPreset'
                                try:
                                    sheet.update_cell(len(sheet.get_all_values()), 4, cmdlist)
                                ...
                                if cmdans == False:
                                    return 
                                if version == 'macos':
                                    getcmd = './DDM /1:ReadColorPreset /2:ReadColorPreset'
                                else:
                                    getcmd = 'DDM /1:ReadColorPreset /2:ReadColorPreset'
                                try:
                                    sheet.update_cell(len(sheet.get_all_values()), 4, cmdlist)
                        ...
                        backup_dict[number][4] = cmdans
                    if mode == 'auto':
                        if 'WriteActiveInput' in cmdlist:
                            if '2:' in cmdlist:
                                if version == 'macos':
                                    self.typing("./DDM /2:WriteActiveInput "+subinput3)
                                else:
                                    self.typing("DDM /2:WriteActiveInput "+subinput3)
                            if version == 'macos':
                                self.typing("./DDM /1:WriteActiveInput "+subinput2)
                            else:
                                self.typing("DDM /1:WriteActiveInput "+subinput2)
                        if 'PIP' in ddct[i][0] or 'PBP' in ddct[i][0]:
                            if version == 'macos':
                                self.typing("./DDM /WritePxP Off")
                            else:
                                self.typing("DDM /WritePxP Off")
                        if 'Language' in ddct[i][0]:
                            if version == 'macos':
                                self.typing('./DDM /1:WriteOSDLanguage English')
                            else:
                                self.typing("DDM /1:WriteOSDLanguage English")
